...
def mock_dependencies(monkeypatch, request):
    ...
    class MockOpenAIResponse:
        def __init__(self, content=openai_content):
            self.choices = [
                type('MockChoice', (), {
    ...
    mock_completions.create.return_value = MockOpenAIResponse()
    ...
    # Create a factory for mock responses to allow different responses per call
    def create_response_factory(predefined_response):
        """Creates a factory that returns the predefined response or customized ones"""
    ...
    for method in ['get', 'post', 'put', 'delete', 'patch']:
        monkeypatch.setattr(f"requests.{method}", create_response_factory(mock_response))
